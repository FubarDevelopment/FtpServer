...
namespace FubarDev.FtpServer.ListFormatters
{
    ...
    public class FactsListFormatter : IListFormatter
    {
        ...
        private IEnumerable<IFact> BuildFacts(IUnixDirectoryEntry? parentEntry, IUnixDirectoryEntry currentEntry, TypeFact typeFact)
        {
            yield return new PermissionsFact(_user, parentEntry, currentEntry);
            yield return typeFact;
            if (currentEntry.LastWriteTime.HasValue)
            {
                yield return new ModifyFact(currentEntry.LastWriteTime.Value);
            }

            if (currentEntry.CreatedTime.HasValue)
            {
                yield return new CreateFact(currentEntry.CreatedTime.Value);
            }
        }

        private IEnumerable<IFact> BuildFacts(IUnixFileSystem fileSystem, IUnixDirectoryEntry directoryEntry, IUnixFileEntry entry)
        {
            yield return new PermissionsFact(_user, fileSystem, directoryEntry, entry);
            yield return new SizeFact(entry.Size);
            yield return new TypeFact(entry);
            if (entry.LastWriteTime.HasValue)
            {
                yield return new ModifyFact(entry.LastWriteTime.Value);
            }

            if (entry.CreatedTime.HasValue)
            {
                yield return new CreateFact(entry.CreatedTime.Value);
            }
        }
    }
}
